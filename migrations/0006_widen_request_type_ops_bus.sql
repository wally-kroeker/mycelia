-- Migration 0006: Widen requests.request_type CHECK for v1.2 ops-bus types,
-- and seed the matching capability tags IN THE SAME MIGRATION.
--
-- Adds six ops-bus request types alongside the original eight eval-surface types:
...
-- If the type enum is widened without the matching tags, agents can select an
-- ops-bus request_type but have no honest tag to attach — in a production
-- fleet deployment this locked an agent out of the bus for hours. Enum and
-- tag taxonomy must never drift; a single migration makes drift impossible.
--
-- SQLite doesn't support ALTER COLUMN or DROP CONSTRAINT, so the CHECK change
-- rebuilds the table. Rebuild order is create-new → copy → drop-old → rename:
-- renaming the ORIGINAL table first (the more common pattern) makes SQLite
-- rewrite the FK clauses of every referring table (request_tags, claims,
-- responses) to point at the temporary name, which then dangles once the
-- temporary table is dropped. The order below never renames a table that
-- other tables reference, so only `requests` is touched and all FK clauses,
-- and all indexes on other tables (including the partial unique index
-- idx_claims_request_agent_active from migration 0003), survive untouched.
...
-- 5. Seed the six ops-bus capability tags — same migration, on purpose (see
...
INSERT OR IGNORE INTO capabilities (tag, category, description, created_at) VALUES
  ...
  ('ack-close',      'ops-bus', 'Ops-bus: acknowledge and wrap up a coordination thread',         datetime('now')),
